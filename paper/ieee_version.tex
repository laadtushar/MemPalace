...
Total cost: $O(n)$ embedding calls $+$ $O(k)$ LLM calls, where $k$ is a fixed constant. The contribution is not in any individual model's capability but in structuring the interaction between embedding and LLM to make belief-evolution analysis tractable at scale. In our evaluation ($n{=}127$ beliefs), this required 127 embedding calls and at most 20 LLM calls, compared to 8,001 LLM calls under the naive approach, a reduction of 99.75\%.
...
To supplement the single-participant case study, we ran a lightweight evaluation with six additional participants.
...
\textbf{Toward the research question.} Both the case study and user study offer early evidence that a computational system can surface belief evolution that users themselves had not explicitly articulated. Both results suggest the system can surface self-knowledge that a search interface alone would not recover. A controlled study with more participants and longitudinal follow-up is needed to establish generalizability.
